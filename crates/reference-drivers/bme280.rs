...
impl<B> Bme280Sensor<B> {
    pub fn new(bus: B) -> Self {
        Self::new_with_config(bus, Bme280Config::default())
    }
    ...
    pub fn new_with_config(bus: B, config: Bme280Config) -> Self {
        Self {
            bus,
            config,
            calibration: None,
    ...
    pub fn config(&self) -> Bme280Config {
        self.config
    }
    ...
    B: I2cBus<Error = I2cError>,
{
    fn initialize(&mut self) -> Result<(), SensorError> {
        ...
        self.write_register(REG_CTRL_HUM, self.config.ctrl_hum)?;
        self.write_register(REG_CTRL_MEAS, self.config.ctrl_meas)?;
    ...
    fn read_raw_sample(&mut self) -> Result<(i32, i32, i32), SensorError> {
        ...
        if adc_temperature == 0x0008_0000 || adc_humidity == 0x8000 {
            return Err(SensorError::InvalidReading);
        }
    ...
    B: I2cBus<Error = I2cError>,
{
    ...
    fn read(&mut self) -> Result<EnvReading, Self::Error> {
        ...
        let mut conversion_in_progress = false;
        let mut polls_remaining = MAX_STATUS_POLLS;
        while self.read_u8(REG_STATUS)? & 0x01 != 0 {
        ...
        let (adc_temperature, adc_pressure, adc_humidity) = match self.read_raw_sample() {
            Ok(sample) => sample,
            Err(SensorError::InvalidReading) if conversion_in_progress => {
...
mod tests {
    ...
    #[test]
    fn bme280_sensor_uses_custom_config_registers() {
        let bus = RecordingI2c::with_bme280_defaults();
        let writes = bus.writes.clone();
        let config = Bme280Config {
            address: BME280_ADDRESS_SECONDARY,
            ctrl_hum: 0x02,
            ctrl_meas: 0x29,
            config: 0x10,
        };
        let mut sensor = Bme280Sensor::new_with_config(bus, config);

        sensor.read().unwrap();

        assert_eq!(sensor.address(), BME280_ADDRESS_SECONDARY);
        assert_eq!(sensor.config(), config);
        assert!(writes
            .borrow()
            .iter()
            .any(|bytes| bytes.as_slice() == [REG_CTRL_HUM, 0x02]));
        assert!(writes
            .borrow()
            .iter()
            .any(|bytes| bytes.as_slice() == [REG_CTRL_MEAS, 0x29]));
        assert!(writes
            .borrow()
            .iter()
            .any(|bytes| bytes.as_slice() == [REG_CONFIG, 0x10]));
    }
}
