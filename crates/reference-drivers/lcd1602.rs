...
pub struct Lcd1602Config {
    pub address: u8,
    pub mapping: BackpackMapping,
    pub backlight: bool,
}

impl Default for Lcd1602Config {
    fn default() -> Self {
        Self {
...
impl<B, D> Lcd1602Display<B, D> {
    pub fn new(bus: B, delay: D) -> Self {
        Self::new_with_config(bus, delay, Lcd1602Config::default())
    }
    ...
    pub fn new_with_config(bus: B, delay: D, config: Lcd1602Config) -> Self {
        Self {
            bus,
            delay,
            config,
    ...
    pub fn config(&self) -> Lcd1602Config {
        self.config
    }

    pub fn backlight_enabled(&self) -> bool {
        self.config.backlight
    }
    ...
    D: DelayNs,
{
    ...
    fn write_init_nibble(&mut self, nibble: u8) -> Result<(), DisplayError> {
        let byte = self
            .config
            .mapping
            .encode_nibble(nibble & 0x0F, false, self.config.backlight);
    ...
    fn write_nibble(&mut self, nibble: u8, register_select: bool) -> Result<(), DisplayError> {
        let byte = self.config.mapping.encode_nibble(
            nibble & 0x0F,
            register_select,
            self.config.backlight,
        );
    ...
    fn pulse_enable(&mut self, byte: u8) -> Result<(), DisplayError> {
        self.bus
            .write(self.config.address, &[byte | self.config.mapping.enable])
...
mod tests {
    ...
    fn lcd1602_display_accepts_custom_config() {
        ...
        let config = Lcd1602Config {
            address: LCD1602_ADDRESS_SECONDARY,
            mapping,
            backlight: false,
        };
        let display = Lcd1602Display::new_with_config(bus, delay, config);

        assert_eq!(display.address(), LCD1602_ADDRESS_SECONDARY);
        assert_eq!(display.config(), config);
        assert!(!display.backlight_enabled());
    }
}
